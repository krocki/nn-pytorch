...
def gradCheck(inputs, target, cprev, hprev, mprev, rprev):
  ...
  for param,dparam,name in zip([Wxh, Whh, Why, Whr, Whv, Whw, Whe, bh, by], [dWxh, dWhh, dWhy, dWhr, dWhv, dWhw, dWhe, dbh, dby], ['Wxh', 'Whh', 'Why', 'Whr', 'Whv', 'Whw', 'Whe', 'bh', 'by']):
    s0 = dparam.shape
    s1 = param.shape
...
def lossFun(inputs, targets, cprev, hprev, mprev, rprev):
  ...
  ### ext
  dWhr, dWhv, dWhw, dWhe = np.zeros_like(Whr), np.zeros_like(Whv), np.zeros_like(Whw), np.zeros_like(Whe)
  ###
...
def sample(c, h, m, r, seed_ix, n):
  ...
  for t in xrange(n):
    ...
    h = g[N:2*N,:] * c
    y = np.dot(Why, h) + by
    mem_new_content = sigmoid(np.dot(Whv, h))
...
while t < T:
  ...
  for param, dparam, mem in zip([Wxh, Whh, Why, Whr, Whv, Whw, Whe, bh, by],
                                [dWxh, dWhh, dWhy, dWhr, dWhv, dWhw, dWhe, dbh, dby], 
                                [mWxh, mWhh, mWhy, mWhr, mWhv, mWhw, mWhe, mbh, mby]):
    mem += dparam * dparam
