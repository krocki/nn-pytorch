...
parser.add_argument('--memrandn', action='store_const', const=True, default=False, help='init memory with random vals')
...
Whr = np.random.randn(mem_width, hidden_size).astype(datatype)*0.01 # read strength
Whw = np.random.randn(mem_width, hidden_size).astype(datatype)*0.01 # write strength
...
def lossFun(inputs, targets, cprev, hprev, mprev, rprev):
  ...
  # external mem
  mem_new_content, mem_read_gates, mem_write_gates, mem_erase_gate, memory, rs = {}, {}, {}, {}, {}, {}
  mem_read_key, mem_write_key, mem_content_read_sim, mem_content_write_sim = {}, {}, {}, {}
  ...
  for t in xrange(len(inputs)):
    ...
    mem_content_read_sim[t] = np.zeros((mem_locations, B), dtype=datatype)
    mem_content_write_sim[t] = np.zeros((mem_locations, B), dtype=datatype)
    mem_read_gates[t] = np.zeros((mem_locations, B), dtype=datatype)
    mem_write_gates[t] = np.zeros((mem_locations, B), dtype=datatype)
    memory[t] = np.zeros((mem_locations, mem_width, B), dtype=datatype)
    rs[t] = np.zeros((mem_width, B), dtype=datatype)
    ...
    # todo, parallelize
    for b in range(0,B):
        mem_content_read_sim[t][:,b] = np.dot(memory[t-1][:,:,b], mem_read_key[t][:,b].T)
        mem_content_write_sim[t][:,b] = np.dot(memory[t-1][:,:,b], mem_write_key[t][:,b].T)
        #for now - sigmoids - todo softmax
        mem_write_gates[t][:,b] = sigmoid(mem_content_write_sim[t][:,b])
        mem_read_gates[t][:,b] = sigmoid(mem_content_read_sim[t][:,b])

        memory[t][:,:,b] = memory[t-1][:,:,b] * (1 - np.dot(mem_write_gates[t][:,b, None], mem_erase_gate[t][:,b, None].T))
        memory[t][:,:,b] += np.dot(mem_write_gates[t][:,b, None], mem_new_content[t][:,b, None].T)
        rs[t][:,b] = np.dot(mem_read_gates[t][:,b,None].T, memory[t][:,:,b])
...
mprev = np.zeros((mem_locations, mem_width, B), dtype=datatype)
rprev = np.zeros((mem_width,B), dtype=datatype)
...
while t < T:
  ...
  for b in range(0,B):
      if p[b]+seq_length+1 >= len(data) or n == 0:
        ...
        if opt.memrandn: mprev[:,:,b] = np.random.randn(mem_locations, mem_width) # reset ext memory
        else: mprev[:,:,b] = np.zeros((mem_locations, mem_width), dtype=datatype) # reset ext memory
